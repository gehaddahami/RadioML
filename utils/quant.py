...
def get_float_state_space(bits: int, scale_factor: float, signed: bool, narrow_range: bool, quant_type):
...
class QuantBrevitasActivation(nn.Module):
    ...
    def get_quant_type(self):
        ...
        '''
        Returns the quantization type used in the activation function 
        in QuantReLU, the default quantization type is int8 hence if we dont decide the quantization type it should return that the quantization type is: "QuantType.INT" 
        otherwise, if the quantization type is explicitly defined, then the quantization type might differ
        '''
    ...
    # Check if necessary quantization methods are present
    ...
    def get_state_space(self):
        ...
        if quant_type == QuantType.INT:
            narrow_range = self.brevitas_module.is_quant_act_narrow_range
            signed = self.brevitas_module.is_quant_act_signed
